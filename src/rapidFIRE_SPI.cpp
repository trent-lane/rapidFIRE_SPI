...
RF_RESULT rapidFIRE_SPI::begin(int pin_SCK, int pin_DATA, int pin_SS, int freq)
{
...
RF_RESULT rapidFIRE_SPI::sendCommand(uint16_t command, byte *data, size_t len)
{
  ...
  spi->begin(SPI_pin_SCK, -1, SPI_pin_DATA, SPI_pin_SS);
  spi->setFrequency(SPI_freq);
  spi->setDataMode(SPI_MODE0);
  spi->setBitOrder(MSBFIRST);
  spi->setHwCs(true);
...
{
  return sendCommand(RAPIDFIRE_CMD_SET_OSD_USER_TEXT, (byte *)text, strlen(text));
}
